...
def async_analysis_result(request):
    ...
    if rsp_data is None:
        return get_json_response(request, dict(status='running', message='data_clear is running.', data=None))
    else:
        ...
        print get_json_response(request, dict(status='ok', message='success.', data=result))
